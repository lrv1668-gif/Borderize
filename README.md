...
Install the published tool from [nuget.org](https://www.nuget.org/packages/Borderize):

```sh
dotnet tool install --global Borderize
```

Update to the latest version later with:

```sh
dotnet tool update --global Borderize
```

### From source

To run a local build — for testing changes or [contributing](CONTRIBUTING.md):
...
If `borderize` isn't found after installing, add the .NET tools directory to your PATH (one-time setup):
